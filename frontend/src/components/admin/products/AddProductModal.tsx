...
import { createProductSchema } from "../../../../../shared/index.js"
import type { CreateProductInput, CreateProductCredentials } from "../../../types/product.types"
...
const AddProductModal = ({ onClose }: AddProductModalProps) => {
  ...
  // 3 genéricos: entrada del form (price string), contexto, salida ya validada (price number)
  const { register, handleSubmit, formState: { errors }, reset } = useForm<
    CreateProductInput, any, CreateProductCredentials
  >({
  ...
  async function onSubmit(data: CreateProductCredentials) {
    try {
      ...
      setErrorResponse(error.response?.data?.message ?? "No pudimos crear el producto. Intentá nuevamente.")
    }
  }
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.35)] overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        ...
        <div className="px-8 pt-10 pb-6 text-center">
          <div className="mx-auto w-16 h-16 rounded-full bg-linear-to-br from-[#9B2335] to-[#4A0E18] flex items-center justify-center mb-5 shadow-lg">
            <i className="bi bi-bag-plus text-white text-2xl" aria-hidden="true" />
          </div>
          <h2 className="text-[#1C1714] text-xl font-bold tracking-wide">
            Nuevo producto
          </h2>
          <p className="text-[#7A6B63] text-sm mt-2 leading-relaxed">
            Cargá un producto en el catálogo y, si querés, subí una imagen.
          </p>
        </div>
