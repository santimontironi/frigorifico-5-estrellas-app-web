...
const AddEmployeeModal = ({ open, onClose }: AddEmployeeModalProps) => {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<EmployeeRegisterCredentials>({
  ...
  async function onSubmit(data: EmployeeRegisterCredentials) {
    try {
      ...
      setErrorResponse(error.response?.data?.message ?? "No pudimos crear el administrador. Intentá nuevamente.")
    }
  }
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-[0_20px_60px_rgba(0,0,0,0.35)] overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        ...
        <div className="px-8 pt-10 pb-6 text-center">
          <div className="mx-auto w-16 h-16 rounded-full bg-linear-to-br from-[#9B2335] to-[#4A0E18] flex items-center justify-center mb-5 shadow-lg">
            <i className="bi bi-person-plus text-white text-2xl" aria-hidden="true" />
          </div>
          <h2 className="text-[#1C1714] text-xl font-bold tracking-wide">
            Nuevo administrador
          </h2>
          <p className="text-[#7A6B63] text-sm mt-2 leading-relaxed">
            Creá una cuenta para gestionar los pedidos del panel.
          </p>
        </div>
        ...
        <div className="px-8 pt-6 pb-10">
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[#7A6B63] text-xs font-medium ml-1">Contraseña</label>
              <div className="relative">
                <i className="bi bi-lock absolute left-4 top-1/2 -translate-y-1/2 text-[#B8A898]" aria-hidden="true" />
                <input
                  type="password"
                  {...register("password")}
                  className="w-full bg-[#F7F4F1] rounded-xl text-[#1C1714] text-sm pl-11 pr-4 py-3.5 outline-none border-2 border-transparent focus:border-[#9B2335]/30 focus:bg-white transition-all duration-200 placeholder:text-[#B8A898]/60"
              ...
              {errors.password && (
                <span className="text-[#9B2335] text-xs ml-1">{errors.password.message}</span>
              )}
            </div>
