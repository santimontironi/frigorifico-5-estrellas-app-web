...
class ProductController {
  ...
  async importFromExcel(req, res) {
    ...
    if (rows.length === 0) {
      // el archivo existe pero no tiene filas de datos
      return res
        .status(400)
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const unit = String(row.unit ?? "")
        .trim()
        .toLowerCase(); // unidad normalizada a minúsculas
      ...
      if (!categoryName) {
        errors.push(`Fila ${rowNum}: falta la categoría`);
        continue;
      } // campo vacío
      if (!["kg", "unit"].includes(unit)) {
        // valor fuera del enum del modelo
        errors.push(
          `Fila ${rowNum}: unidad inválida "${unit}" (debe ser kg o unit)`,
        );
        continue;
      }
      if (isNaN(price) || price < 0) {
        // no es número o es negativo
